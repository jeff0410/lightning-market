...
export function TestPage() {
  ...
  const { sort, keyword, dataList, suggestions, doSearch, setKeyword, setSort, clearSuggestions } = useJeffTest()
  ...
  return (
    <div className="p-8 flex flex-col gap-8 w-full">
      ...
      <div className="w-full bg-gray-50 flex flex-col gap-4">
        {/* 검색부분 */}
        <SearchBar
          keyword={keyword}
          doSearch={doSearch}
        ...
        <div className="grid grid-cols-4 gap-4">
          {dataList.length > 0 && dataList.map((val, index) => (
            <ContentItem item={val} key={index} />
          ))}
        </div>
      </div>
    </div>
